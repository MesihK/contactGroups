...
def getColDict(mscore, pmap):
    return dict((v, [cp.scorerdict[a] for a in mscore[:,v]]) for v in list(pmap.keys()))
...
def report_hatvar(v1,v2, cdict, rdict, pmap, ensemble):
    ...
    ht_12 = np.array([(cp.aaprop[c1[i]][21] + cp.aaprop[c2[i]][21]) for i in range(0, len(c1))])
    std12 = np.std(ht_12)
...
def report_hat_entropy(v1,v2, cdict, rdict, pmap, ensemble):
    ...
    # one column data
    ht_12 = np.array([math.fabs(cp.aaprop[c1[i]][21] - cp.aaprop[c2[i]][21]) for i in range(0, len(c1))])

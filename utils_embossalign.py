...
class embossalign(object):
    ...
    def getAlignedpos(self):
        alnpos = [i for i in range(0, self.alignlen) if (self.seqA[i] == self.seqB[i]) and (self.seqA[i] not in cp.gaps)]
    ...
    def dump(self):
        ...
        print(('            %s' % (''.join(['+' if (self.seqA[i] == self.seqB[i] and self.seqA[i] not in cp.gaps) else '.' for i in range(0,len(self.seqA)) ]))))
    ...
    def getreadable(self):
        return ''.join([
                ...
                ('seq-B-len:  %d\n' % self.seqBlen),
                ('aligned-A:  %s\n' % self.seqA),
                ('            %s\n' % (''.join(['|' if (self.seqA[i] == self.seqB[i] and self.seqA[i] not in cp.gaps) else '.' for i in range(0,len(self.seqA)) ])) ),
                ('aligned-B:  %s\n' % self.seqB)
        ])

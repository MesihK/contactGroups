...
def tupleqij(arglist):
    ...
    for c, group in itertools.groupby(plist, lambda x: (x[0], x[1])):
        ...
        print '-----------------------'
        '''
    aakey = ['%s%s' % (cp.aas01[i],cp.aas01[j]) for i in range(0, len(cp.aas01)) for j in range(i, len(cp.aas01))]
...
def single_210x400(arglist):
    ...
    dictall = dict( ('%s%s' % (cp.aas01[i],cp.aas01[j]), defaultdict(lambda:0)) for i in range(0,len(cp.aas01)) for j in range(i+1, len(cp.aas01)) )
...
def qij400(args):
    ...
    for msai2, freq2list in itertools.groupby(cp.loadtuples(freq2file), lambda x: (x[0], x[1])):
        ...
        k = [aa for aa in list(aafreq.keys()) if '.' not in aa] # remove gap
        k.sort()
        # off-diagonal terms
        for i in range(0, len(k)):
            for j in range(i+1, len(k)):

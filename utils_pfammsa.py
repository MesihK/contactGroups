...
def aafreq(arglist):
    ...
    output =[(k, v, float(v)/freqsum) for k,v in list(freqdict.items()) if v > 0]
...
def scoreentropy(arglist):
    ...
    scorefile = arglist[0]
    score = np.loadtxt(scorefile, delimiter=',')
    hlist = [cp.entropy([score[:,i]]) for i in range(0, score.shape[1])]
...
def tuplesubfreq(args):
    ...
    cdata = np.array([list(map(''.join, list(zip(*npmsa[:,s].T)))) if len(s) != 1 else npmsa[:,s[0]].T for s in vlist])
    print(cdata)

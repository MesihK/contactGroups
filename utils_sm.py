...
class smatrix(object):
    ...
    def out210vec(self):
        return np.array([self.core[i][j] for i in range(20) for j in range(i,20)])
    ...
    def score2core(self):
        ...
        scoreupdate = []
        for i in range(20):
            scoreupdate.append([self.score['%s%s' %(self.aa[i],self.aa[j])] for j in range(20)])
...
def gabreed(arglist):
    ...
    for p in range(len(smlist)):
        ...
        permu = permulist[p]
        outsm = copy.deepcopy(sm)
        for k in range(ncm): # generate k cross breeds for one sm
...
def printflatten(arglist):
    ...
    smfile = arglist[0]
    sm = smatrix(smfile)
    print('%s' % ' '.join(['%i' % (sm.core[i][j]) for i in range(20) for j in range(i,20)]))
...
def outblast(arglist):
    ...
    blastsm = ',\n'.join(['\t%s' % (','.join(['%3i' % n for n in cp.b80blast[i,:]])) for i in range(len(cp.aablast))])
...
def outblast62(arglist):
    ...
    blastsm = ',\n'.join(['\t%s' % (','.join(['%3i' % n for n in cp.b62blast[i,:]])) for i in range(len(cp.aablast))])
...
def savecolvec(arglist):
    ...
    outstr = ['%s%s %d' % (cp.aat01[i], cp.aat01[j], sm.score[cp.aat01[i]+cp.aat01[j]]) for i in range(0, len(cp.aat01)) for j in range(i, len(cp.aat01))]
    with open(outfile, 'w') as fout:
        fout.write('%s\n' % ('\n'.join(outstr)))
    cp._info('save to %s' % outfile)

...
class palign(object):
    ...
    def alnpos(self):
        gap = ['.', '-']
        poslist = []
        for i in range(0, self.alignlen):
    ...
    def alnposlist(self):
        ...
        for i in range(0, self.alignlen):
            if (self.seqA[i] not in gap) and (self.seqB[i] not in gap):
                ...
                if forward > start:
                    setlist.append([k for k in range(start+1, forward+1)])
                start = i
        #print repr(setlist)
        return setlist
...
def aln2color(arglist):
    ...
    start=0
    forward=0
    for i in range(0, len(msaseqA)):
        if (msaseqA[i] not in cp.gaps) and (msaseqB[i] not in cp.gaps):
            forward=i
        else:
            if forward > start:
                alnposlist.append([k for k in range(start+1, forward+1)])
            start = i
    ...
    for k in range(0, len(alnposlist)):
        pAresi=[]
        pBresi=[]
...
def aln2rmsd(arglist):
    ...
    alnpos = [i for i in range(0, len(msaseqA)) if ((msaseqA[i] not in cp.gaps) and (msaseqB[i] not in cp.gaps))]
...
def alignparse(title, alignstr):
    lines = [_f for _f in alignstr.split('\n') if _f]
...
def cvepoint(arglist):
    ...
    for t in np.arange(0, mepq, interval):
        for i in range(pointer, len(cvelist)):
            if cvelist[i][0] <= t:
                continue
            else:
                pointer = i
...
def pairall2all(arglist):
    ...
    outstr = ['p.%s.%s.seq' % (namelist[i], namelist[j]) for i in range(0,len(namelist)) for j in range(i+1, len(namelist))]
